...
router.post("/upload", async (req, res) => {
  try {
    ...
    const stored = await storeDataUri(buffer, public_id);
...
/**
 * Gemini surfaces auth problems as a raw JSON blob inside error.message.
 * Translate the common cases into something the UI can actually show a user,
 * and never leak the provider payload straight through.
 */
function aiError(res: any, error: any) {
  const raw = String(error?.message || "");
  const isAuth = raw.includes('"code":401') || raw.includes("UNAUTHENTICATED") ||
                 raw.includes('"code":403') || raw.includes("API_KEY_INVALID");
  const isQuota = raw.includes('"code":429') || raw.includes("RESOURCE_EXHAUSTED");

  if (isAuth) {
    console.error("[ai] Gemini rejected the credentials — check API_KEY_GEMINI.", raw.slice(0, 200));
    return res.status(503).json({
      message:
        "The AI service is not configured correctly. Please set a valid Gemini API key (API_KEY_GEMINI).",
    });
  }
  if (isQuota) {
    console.error("[ai] Gemini quota exhausted.", raw.slice(0, 200));
    return res.status(429).json({
      message: "The AI service is temporarily rate limited. Please try again shortly.",
    });
  }
  console.error("[ai]", raw.slice(0, 300));
  return res.status(500).json({ message: "Could not complete the AI request. Please try again." });
}
